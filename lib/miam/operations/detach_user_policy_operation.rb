module Miam
  module Operations
    class DetachUserPolicyOperation < Operation
      ...
      def call(args)
        ...
        Output.new(
          user_name: user_name, policy_name: policy_name, detached: true
        )
      rescue Aws::DynamoDB::Errors::TransactionCanceledException
        raise OperationError, 'Unable to detach policy, user/policy does not exists or policy is not attached'
      end
    end
  end
end
