module Miam
  module Operations
    class AttachUserGroupOperation < Operation
      ...
      def call(args)
        ...
        Output.new(
          user_name: user_name, group_name: group_name, attached: true
        )
      rescue Aws::DynamoDB::Errors::TransactionCanceledException
        raise OperationError, 'Unable to attach group, user/group does not exists OR group is already attached to that user'
      end
    end
  end
end
